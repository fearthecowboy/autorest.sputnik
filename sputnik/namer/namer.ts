...
function cloneLanguageInfo(graph: any) {
  // make sure recursively that every language field has sputnik language info
  for (const { index, instance } of visitor(graph)) {
    if (index === 'language' && instance.default && !instance.sputnik) {
      instance.sputnik = clone(instance.default, false, undefined, undefined, ['schema', 'origin']);
    }
  }
}
...
export async function process(session: Session<CodeModel>) {
  ...
  if (model.language.sputnik) {
    ...
    throw Error('Sputnik Namer Failed');
  }

  // copy all the .language.default data into .language.sputnik
  cloneLanguageInfo(model);

  // go set the names of things you care about
  for (const schema of values(model.schemas.objects)) {
    const details = <Language>schema.language.sputnik;
